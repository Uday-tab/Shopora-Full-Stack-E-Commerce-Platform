...
const ProductService = (() => {
  const getAll = (filters = {}) => {
    let items = ShoporaDB.getAll('products').filter(p => p.status === 'published');
    ...
    if (filters.minPrice !== undefined) items = items.filter(p => _effectivePrice(p) >= filters.minPrice);
    if (filters.maxPrice !== undefined) items = items.filter(p => _effectivePrice(p) <= filters.maxPrice);
    if (filters.minRating) items = items.filter(p => _avgRating(p) >= filters.minRating);
    if (filters.sellerId) items = items.filter(p => p.sellerId === filters.sellerId);

    if (filters.sort) {
      switch(filters.sort) {
        case 'price-low':  items.sort((a,b) => _effectivePrice(a) - _effectivePrice(b)); break;
  ...
  const getAllIncludingDrafts = (sellerId) => ShoporaDB.getAll('products').filter(p => p.sellerId === sellerId);
  ...
  const search = (keyword) => {
    ...
    return ShoporaDB.getAll('products').filter(p => {
      ...
      const haystack = `${p.title} ${p.brand} ${p.category} ${p.subcategory || ''} ${p.description} ${(p.tags||[]).join(' ')}`.toLowerCase();
  ...
  const autocomplete = (keyword) => {
    ...
    const results = ShoporaDB.getAll('products').filter(p => p.status === 'published' && p.title.toLowerCase().includes(kw));
    return results.slice(0, 6).map(p => ({ id: p.id, title: p.title, price: _effectivePrice(p) }));
  };
  ...
  const create = (productData) => {
    ...
    if (seller) ShoporaDB.update('sellers', seller.id, { productsCount: (seller.productsCount || 0) + 1 });
    return product;
  };

  const updateProduct = (id, patch) => {
    if (patch.variants) patch.stock = patch.variants.reduce((s, v) => s + (v.stock || 0), 0);
  ...
  const addReview = (productId, review) => {
    ...
    return ShoporaDB.update('products', productId, { reviews: p.reviews, ratings: p.ratings });
  };

  const getRecommendations = (productId, limit = 4) => {
    const p = getById(productId);
    if (!p) return [];
    return ShoporaDB.getAll('products').filter(x => x.id !== productId && x.status === 'published' && (x.category === p.category || (p.tags || []).some(t => (x.tags || []).includes(t)))).slice(0, limit);
  };

  const getCategories = () => {
    const prods = ShoporaDB.getAll('products').filter(p => p.status === 'published');
  ...
  return { getAll, getAllIncludingDrafts, getById, search, autocomplete, create, updateProduct, deleteProduct, addReview, getRecommendations, getCategories, getBrands, getEffectivePrice, getAvgRating };
})();
