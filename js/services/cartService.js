...
const CartService = (() => {
  ...
  const addToCart = (productId, variant = null, qty = 1) => {
    ...
    const maxStock = (variant && typeof variant.stock === 'number')
      ? variant.stock
      : (product.stock || 0);
  ...
  const updateQuantity = (index, qty) => {
    ...
    if (qty <= 0) {
      ...
      const maxStock = product
        ? ((item.variant && typeof item.variant.stock === 'number')
            ? item.variant.stock
            : (product.stock || 0))
        : qty;
  ...
  const getItemCount = () => {
    return _get().reduce((sum, item) => sum + item.quantity, 0);
  };

  const getSubtotal = () => {
    return _get().reduce(
      (sum, item) => sum + (item.variantPrice || item.effectivePrice) * item.quantity,
      0
    );
  };
  ...
  const getSavings = () => {
    return _get().reduce(
      (sum, item) => sum + (item.price - (item.variantPrice || item.effectivePrice)) * item.quantity,
      0
    );
  };
